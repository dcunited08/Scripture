...
if(strstr($mobl,'ipad')or strstr($mobl,'windows')or strstr($mobl,'linux')){$no_mobile='1';}
elseif(strstr($mobl,'ipad')or strstr($mobl,'iphone')or strstr($mobl,'mac os x')){$iphone=1;}
if(strstr($mobl,'mobi')){unset($no_mobile);}
/*
...
elseif(isset($_GET['forum'])){$forum=stripslashes($_GET['forum']);}
elseif(isset($_GET['groups'])){$groups=stripslashes($_GET['groups']);}
if(!empty($_GET['b'])){$b=$_GET['b'];}
 elseif(!empty($_POST['b'])){$b=$_POST['b'];}
 else{
  if(empty($_SERVER['QUERY_STRING'])){$emptyquery=1;$fp='1';#setting menu as frontpage
  }
  elseif(isset($_GET['br'])){
    #$doexpro=stripslashes($_SERVER['HTTP_REFERER']);
    parse_str($referer,$doexprop);
    if(!empty($doexprop['b'])){$b=$doexprop['b'];}
  }
 }
...
if(empty($_POST['loguser'])&&empty($firsttime)){// user check
  $thelanguage=stripslashes($_SERVER['HTTP_ACCEPT_LANGUAGE']);
  $l=strtolower(substr($thelanguage,0,2));
if($l=='no'){$l='n';}
...
if(!isset($nocount)and($uid!=='1')and!preg_match('/(facebookexternalhit|bingbot|linkfinder|spider|Google|msnbot|Rambler|Yahoo|Yammybot|openbot|AbachoBOT|accoona|AcioRobot|ASPSeek|Dumbot|Crawler|GeonaBot|Gigabot|Lycos|MSRBOT|Scooter|AltaVista|IDBot|eStyle|Scrubby|DoCoMo|Teoma|Slurp|archiver)/i',$mobl)){
 ...
 mysql_query_s('INSERT INTO '.$database.'.bible_visitors (visitors_id,visitors_uid,visitors_ip,visitors_date_time,visitors_url,visitors_referer,visitors_path,visitors_title,visitors_user_agent,geodata,new,lang) VALUES '."
  ('',$uid,'$remoteaddr',unix_timestamp(),'".stripslashes($_SERVER['QUERY_STRING'])."','".$referer."','".stripslashes($_SERVER['SCRIPT_NAME'])."','$u2','".$newagent."','$geodata','$new','$sthelanguage')");
...
if(isset($_GET['alv'])or isset($jsdo)){$nojqueryl=1;echo'<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.js"></script>';if(!isset($jsdo)){require('inc/ajax/admin_lastvisitor_js.php');}}
if(isset($_GET['tea'])){if(isset($no_mobile)){echo'<script type="text/javascript" src="inc/jscolor/jscolor.js"></script>';}}
/*if(!isset($no_mobile)){#Perhaps processbar later
if(!isset($nojqueryl)){echo'<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.js"></script>';}
...
$footer='</body></html>';
if(isset($_GET['gs'])){$getstrong=stripslashes($_GET['gs']);}elseif(isset($_GET['getstrong'])){$getstrong=stripslashes($_GET['getstrong']);}else{$getstrong="";}
if(isset($_GET['bk'])){$book=stripslashes($_GET['bk']);}else{$book='GEN';}
if(isset($_GET['xref'])){$xref=stripslashes($_GET['xref']);}
if(isset($_GET['ch'])){$chbf=stripslashes($_GET['ch']);}
if(isset($_GET['setbookmark'])){$setbookmark=stripslashes($_GET['setbookmark']);}
elseif(isset($_GET['bookmark'])){$bookmark=urldecode(stripslashes($_GET['bookmark']));}
if(isset($_GET['uf'])){$userfeatures=urldecode(stripslashes($_GET['uf']));}elseif(isset($_GET['userfeatures'])){$userfeatures=urldecode(stripslashes($_GET['userfeatures']));}
if(isset($_GET['setnote'])){$setnote=stripslashes($_GET['setnote']);}
if(isset($_GET['setnote2'])){$setnote2=stripslashes($_GET['setnote2']);}
if(isset($_GET['setxref'])){$setxref=stripslashes($_GET['setxref']);}
elseif(isset($_GET['setfavorite'])){$setfavorite=stripslashes($_GET['setfavorite']);}
if(isset($_GET['note'])){$note=stripslashes($_GET['note']);}
if(isset($_GET['favorite'])){$favorite=stripslashes($_GET['favorite']);}
if(isset($_GET['cross'])){$cross=stripslashes($_GET['cross']);}
if(isset($_GET['reref'])){$reref=urldecode(stripslashes($_GET['reref']));}
if(isset($_GET['mydata'])){$mydata=urldecode(stripslashes($_GET['mydata']));}
  elseif(isset($_POST['mydata'])){$mydata=urldecode(stripslashes($_POST['mydata']));}
if(isset($_GET['mode_tt'])){$mode_tt=urldecode(stripslashes($_GET['mode_tt']));}
  elseif(isset($_POST['mode_tt'])){$mode_tt=urldecode(stripslashes($_POST['mode_tt']));}
if(empty($mysettings[11])){$mysettings[11]='3';}//default frontpage

if(is_array($b)){
  foreach ($b as $key=>$value){$b[$key]=stripslashes($value);}
...
if(isset($_GET['cs'])){$chap=stripslashes($_GET['cs']);}
elseif(isset($_GET['c'])){$chap=stripslashes($_GET['c']);}elseif(empty($chap)){$chap='1';}
if(isset($_GET['tools'])){$tools=stripslashes($_GET['tools']);}elseif(isset($_POST['tools'])){$tools=urldecode(stripslashes($_POST['tools']));}
if(!empty($chap)){$prechap=$chap - 1;$postchap=$chap + 1;}
if($chbf == '<<'){$chap=$prechap;}elseif($chbf == '>>'){$chap=$postchap;}
$subd=stripslashes($_SERVER['SCRIPT_NAME']);$subd=explode('/',$subd);
...
if(isset($_GET['usim'])){$uid=0;}
if(isset($_GET['mobsim'])){unset($no_mobile);}
if(isset($firsttime)){unset($fp);}
