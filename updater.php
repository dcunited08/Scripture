...
if (($uid == '1')or(!empty($firsttime))) {
    ...
    if (!empty($_POST['install']) or !empty($_GET['updates']) or !empty($majorupdatelink)) {
      ...
      if (isset($_POST['install'])) { $username =stripslashes($_POST['username']);$password =stripslashes($_POST['password']);$database =stripslashes($_POST['database']);$database_host =stripslashes($_POST['database_host']);
				    $website_email =stripslashes($_POST['website_email']);$mailer_name =stripslashes($_POST['mailer_name']); $smtp_host=stripslashes($_POST['smtphost']);$smtp_username=stripslashes($_POST['smtpuser']);$smtp_pass=stripslashes($_POST['smtppass']);}
	  ...
	  $usernameadmin =stripslashes($_POST['usernameadmin']);$passwordadmin =stripslashes($_POST['passwordadmin']);$emailadmin =stripslashes($_POST['emailadmin']);
      }
    }
	...
	else { $toupdate =stripslashes($_GET['updates']); }
