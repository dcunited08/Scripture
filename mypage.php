...
elseif(!empty($_POST['notetitle']) && empty($_POST['lookupnote']) && empty($_POST['newnote'])) {
    ...
    if ($_POST['notetitle'] == $_POST['noted']) { $mysqldo2 = "UPDATE bible_quicknotes SET note = '$noteset',updated= UNIX_TIMESTAMP() WHERE nid = ".$_POST['notenid']." AND uid=$uid";}
...
for($i=0;$i<$num8; ++$i) {
    ...
    echo'<option value="'.$sqlnid2.'">'.$sqltitle2.'</option>';
}
...
echo'<form action="index.php" method="get" enctype="multipart/form-data">
    ...
    $num8=mysql_numrows($result8);
    for($i=0;$i<$num8; ++$i) {
         echo'<option value="'.mysql_result($result8,$i,'line').'">'.substr(mysql_result($result8,$i,'title'),0,100).'</option>';
    }
