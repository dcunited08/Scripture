...
if(!empty($_GET['s'])or!empty($_GET['s2'])or!empty($_GET['s3'])or!empty($bookmark)or!empty($favorite)or!empty($_POST['lookup'])or!empty($_GET['lookup'])or!empty($cross)or!empty($_GET['advs'])or(empty($setnote)and empty($setnote2)and empty($setxref)and !empty($note))){
 ...
 foreach($se3 as $k){$se=trim($k);
	...
	if($treetok=='phi'){$treetok=substr($t_book,0,5);}
	elseif(($treetok=='jud')or($treetok=='fil')){$treetok=$firtok;}//$treetok=preg_match('/(jud[^\s]?)/i',$t_book);}
	/*if($l=='ru'){#russian
	 if(is_numeric(substr($treetok,0,1))){$treetok=$firtok;}
	 elseif(($treetok=='Вто')or($treetok=='Нее')or($treetok=='При')or($treetok=='Екк')or($treetok=='Пес')or($treetok=='Пла')or($treetok=='Оси')or($treetok=='Иои')or($treetok=='Амо')or($treetok=='Ион')or($treetok=='Нау')or($treetok=='Дея')or($treetok=='Отк')){$treetok=substr($t_book,0,4);}
	}*/
	if((!empty($ats[1])and is_numeric($ats[1]))or(!empty($ats[1])and strpos($ats[1],':') >= '1')or(strpos($se,'(') >= '1')){ // or preg_match('/(\d)/',$ats[1])
	  ...
	  #$token=array_search($treetok,$a_shortdrupal); #old way of searching
	  #if($token !== FALSE){$b_name=$treetok;} //+39 to make it the nt books
	  if(empty($token)){require('inc/Languages/Filter_english.php');
	  $ia=0;foreach($a_books as $key=>$ki){if($ki==$treetok){$token=$ia;break;}++$ia;}
	  #if($token !== FALSE){$b_name=$a_shortdrupal[$token];}
	  #$token=array_search($treetok,$a_books);
	  } //oldway for $a_shortdrupal: array_map('strtolower',$a_shortdrupal)
	  
	  if($l=='ru'){if(!isset($a_rubok)){require('inc/Languages/Filter_russian.php');}
	  ...
	  #$token=array_search($treetok,$a_rubok);
	  } //oldway for $a_shortdrupal: array_map('strtolower',$a_shortdrupal)
	  #if(!empty($token)){$b_name=$a_shortdrupal[$token];}
	  if(empty($token)){if(!isset($a_shortdrupal)){require('inc/Languages/Filter_norwegian.php');}
	  #$ia=0;foreach($a_bokers as $key=>$ki){if($ki==$firtok){$token=$ia;break;}++$ia;}
	  $ia=0;foreach($a_bokers as $key=>$ki){if($ki==$treetok){$token=$ia;break;}++$ia;}
	  #$token=array_search(substr($firtok,$a_bokers);
	  }
	  #if(!empty($token)){$b_name=$a_shortdrupal[$token];}
	  if(empty($token)and!empty($a_dbbooksorg)){
	   $ia=0;foreach($a_dbbooksorg as $key=>$ki){if($ki==$treetok){$token=$ia;break;}++$ia;}
	   #$token=array_search($treetok,$a_dbbooksorg);
	  }
	  if(empty($token)){if(!isset($a_sword)){require('inc/Languages/Filter_sword.php');}
	  #$token=array_search($firtok,$a_sword);
	  #if($token==FALSE){$token=array_search($treetok,$a_sword);}
	  #if($token==FALSE){$token=array_search(substr($t_book,0,2),$a_sword);}
...
if(empty($seonly) and empty($nobook)){$w="";$bs="";
  ...
  else {$bse='SELECT  max(rev),revuid,revdate,bid,book,chapter,verse,context,vsid,mode FROM '.$database.'.bible_context '.$wbid.' AND (';
